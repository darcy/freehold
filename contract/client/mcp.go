// Package client is the MCP-over-HTTP client the operator CLI uses to drive a
// Rust runner. Every tools/call body is signed with the agent identity (grants
// enforce it runner-side), reproducing the old orchestrator client.
package client
...
func New(url string, auth *AgentAuth, runnerPubkey string) (*McpClient, error) {
	...
	// A wedged runner must not hang the operator CLI forever.
