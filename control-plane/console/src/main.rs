...
struct GrantArgs {
    ...
    /// identity the operator CLI drives with, so the common first-run grant
    /// needs no argument
    #[arg(long)]
...
struct RevokeGrantArgs {
    ...
    /// ops-agent identity (the operator CLI.s signing identity)
    #[arg(long)]
    pubkey: Option<String>,
...
fn resolve_grant_pubkey(
    ...
    // default: the state dir's own ops-agent identity (the operator CLI.s
