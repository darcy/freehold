...
impl ServerHandler for TestServer {
    ...
    async fn exec_request(
        ...
        // stall the runtime (and the client's timeout timers) for its whole
        // duration.
        let output = tokio::process::Command::new("sh")
            ...
            .output()
            .await?;
        // All sends go through server::Handle; its unit-ish error types are
        // converted to io::Error so they box cleanly.
        session
            .handle()
...
/// Boot an in-process sshd on an ephemeral port; returns the address.
async fn spawn_server(reject_all_keys: bool) -> (std::net::SocketAddr, Arc<ServerState>) {
...
fn client_key_pem() -> (PrivateKey, String) {
