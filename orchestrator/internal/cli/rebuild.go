...
// reconcile — see stages.K3sLocalPathDurableScript). Aborting on failure would sink the
...
func (e *rebuildEngine) stageK3sLocalPath(k3sVmid uint32) {
	ok, out := e.runBin(e.bins.Self, e.execArgs(
		fmt.Sprintf("pct exec %d -- bash -c '%s'", k3sVmid, strings.TrimSpace(stages.K3sLocalPathDurableScript)), 180))
	if !ok {
...
func (e *rebuildEngine) stageK3s() error {
	...
	if (!ok || strings.TrimSpace(out) == "") && !strings.Contains(out, "/usr/local/bin/k3s") {
		ok, out := e.runBin(e.bins.Self, e.execArgs(
			fmt.Sprintf("pct exec %d -- bash -c '%s'", vmid, strings.TrimSpace(stages.K3sInstallScript)), 900))
		if !ok {
...
// The litellm kube manifest builder lives in stages.LitellmManifestScript
// (shared so the CP world_build executor runs the same bytes).

// The litellm postgres/gateway kube manifests + the model-registration curl
// builder live in stages (LitellmManifestScript / LitellmRegisterScript),
// shared so the CP world_build executor runs the same bytes.
...
// The Caddy kube manifest builder lives in stages.CaddyManifestScript (shared
// so the CP world_build executor runs the same bytes).
...
func (e *rebuildEngine) stageCaddy() error {
	...
	ok, out := e.runBin(e.bins.Self, e.execArgs(stages.CaddyManifestScript(k3sVmid, deploy.CaddyManifest(caddyfile)), 180))
	if !ok {
...
func (e *rebuildEngine) sealCertKey(slot string, key []byte) error {
	return e.sealRunnerSecret("cert-key-"+slot, stages.SlotCertKeyEnv(slot), string(key))
}
...
func (e *rebuildEngine) installCaddyCert(k3sVmid uint32, slot string, fullchain []byte) error {
	args := []string{"exec", "--addr", e.f.addr, "--agent-dir", rbOpsDir(),
		...
		stages.CaddyCertInstallScript(k3sVmid, slot, fullchain)}
...
// The cert-slot env var name + durable mirror path live in stages
// (SlotCertKeyEnv / CaddyEdgeDurableDir), shared with the CP executor.
...
func (e *rebuildEngine) caddyDurableFullchain(k3sVmid uint32, slot string) ([]byte, error) {
	cmd := fmt.Sprintf(
		...
		k3sVmid, stages.CaddyEdgeDurableDir(slot), stages.CaddyEdgeDurableDir(slot))
	ok, out := e.runBin(e.bins.Self, e.execArgs(cmd, 60))
...
func (e *rebuildEngine) caddyDurableKeyPresent(k3sVmid uint32, slot string) bool {
	cmd := fmt.Sprintf("pct exec %d -- bash -c 'test -s %s/key.pem'", k3sVmid, stages.CaddyEdgeDurableDir(slot))
...
'`, k3sVmid, slot, stages.CaddyEdgeDurableDir(slot)), 120))
	if !ok {
