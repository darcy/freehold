...
#[tokio::test(flavor = "multi_thread")]
async fn deploy_cp_ships_binary_seeds_identity_and_starts() {
    let base = tempfile::tempdir().unwrap();
    // A tiny fake "control-plane": a real sh script that records a marker on
    // start (proves the SHIPPED bytes were executed), then exits.
    ...
    .unwrap();
    let fake_identity = base.path().join("identity.json");
    std::fs::write(&fake_identity, b"{\"nostr_secret_hex\":\"abc\"}").unwrap();
    ...
    let res = deploy_cp::deploy_cp(
        ...
        &deploy_cp::DeployCpSpec {
            ...
            identity_path: fake_identity.clone(),
    ...
    // Shipped binary == local bytes; identity seeded with 0600.
    ...
    let seeded = std::fs::read(base.path().join("deploy/cp/console/identity.json")).unwrap();
    assert_eq!(
        seeded,
        std::fs::read(&fake_identity).unwrap(),
        "identity bytes exact"
    );
    let mode = std::fs::metadata(base.path().join("deploy/cp/console/identity.json"))
        .unwrap()
        .permissions()
        .mode()
        & 0o777;
    assert_eq!(mode, 0o600, "identity must be 0600 on the box");
...
async fn deploy_cp_refuses_non_loopback_bind() {
    ...
    let fake_id = base.path().join("i");
    std::fs::write(&fake_id, b"{}").unwrap();
