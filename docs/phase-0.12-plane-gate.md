...
`orchestrator/internal/cli/handlers3.go` and
`orchestrator/internal/drive/lvm.go`.
...
`lvremove`, per `DestroyTenantBackend` in `orchestrator/internal/drive/lvm.go`
...
sequence is pinned by `TestDestroy*` in `orchestrator/internal/drive/lvm_test.go`
(~line 479: `umount+fstab-strip must PRECEDE lvremove`).

### Verification

`orchestrator/internal/drive` unit suite (`lvm_test.go`, `lvm_pool_test.go`)
