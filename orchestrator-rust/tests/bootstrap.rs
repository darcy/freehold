...
use freehold_orchestrator_lib::bootstrap::{
...
use freehold_orchestrator_lib::planebase::MountSpec;
use freehold_orchestrator_lib::{deploy_cp, relay_member};
...
async fn proxmox_lxc_reuses_present_template_docker_ready() {
    ...
    assert_eq!(
        res.kind,
        freehold_orchestrator_lib::bootstrap::TargetKind::ProxmoxLxc
    );
    assert!(
...
async fn storage_resolve_reuses_existing_zpool() {
    ...
    let action = freehold_orchestrator_lib::drive::resolve_proxmox(&client, "proxmox-box", false, None)
        .await
        .unwrap();
    assert_eq!(
        action,
        freehold_orchestrator_lib::planebase::ResolveAction::Reuse(
            freehold_orchestrator_lib::planebase::ExistingBackend::Zfs,
            "rpool".into(),
        )
    );
    server.abort();
...
async fn storage_resolve_reuses_existing_lvm_when_no_zpool() {
    ...
    assert_eq!(
        action,
        freehold_orchestrator_lib::planebase::ResolveAction::Reuse(
            freehold_orchestrator_lib::planebase::ExistingBackend::LvmThin,
            "vg".into(),
        )
    );
    server.abort();
...
async fn storage_resolve_bails_without_consent_when_no_backend() {
    ...
    let action = freehold_orchestrator_lib::drive::resolve_proxmox(&client, "proxmox-box", false, None)
        ...
        .unwrap();
    match action {
        freehold_orchestrator_lib::planebase::ResolveAction::Bail(m) => {
            assert!(
...
async fn storage_resolve_with_consent_creates_when_no_backend() {
    ...
    let (_log, _rd, client, server) = proxmox_fixture(base.path(), &bin).await;
    let action = freehold_orchestrator_lib::drive::resolve_proxmox(&client, "proxmox-box", true, None)
        .await
        .unwrap();
    assert_eq!(
        action,
        freehold_orchestrator_lib::planebase::ResolveAction::Create(
            freehold_orchestrator_lib::planebase::Backend::LvmThin,
            "freehold".into(),
        ),
...
async fn ensure_dataset_is_idempotent_and_chown_runs() {
    use freehold_orchestrator_lib::drive::{chown_guest_uid, ensure_dataset};
    let base = tempfile::tempdir().unwrap();
...
async fn destroy_tenant_dataset_destroys_relay_parent_recursively() {
    use freehold_orchestrator_lib::drive::destroy_tenant_dataset;
    let base = tempfile::tempdir().unwrap();
    let logp = base.path().join("zfs.log");
    ...
    destroy_tenant_dataset(
        ...
        freehold_orchestrator_lib::planebase::Tenant::Relay,
    )
    .await
    .unwrap();
...
async fn destroy_tenant_dataset_absent_is_a_noop_not_an_error() {
    ...
    let destroyed = destroy_tenant_dataset(
        ...
        "rpool",
        "freehold-test.darcydev.net",
        freehold_orchestrator_lib::planebase::Tenant::Cp,
    )
    .await
    .unwrap();
    assert!(
...
async fn resolve_lvm_mounts_creates_thin_lvs_for_stock_pve_host() {
    use freehold_orchestrator_lib::drive::resolve_lvm_mounts;
    use freehold_orchestrator_lib::planebase::Tenant;
    let base = tempfile::tempdir().unwrap();
...
async fn destroy_lvm_tenant_unmounts_strips_fstab_then_removes_lvs() {
    use freehold_orchestrator_lib::drive::destroy_lvm_tenant;
    use freehold_orchestrator_lib::planebase::Tenant;
    let base = tempfile::tempdir().unwrap();
    let logp = base.path().join("lvm.log");
...
fn deploy_paths_track_guest_paths() {
    assert!(
        freehold_orchestrator_lib::deploy_cp::DEFAULT_CP_STATE_DIR
            .starts_with(freehold_orchestrator_lib::planebase::GUEST_PATH_CP),
        "cp state dir must live under the cp guest path"
    );
    assert!(
        freehold_orchestrator_lib::deploy_cp::DEFAULT_CP_BIN_DIR
            .starts_with(freehold_orchestrator_lib::planebase::GUEST_PATH_CP),
        "cp bin dir must live under the cp guest path"
    );
    assert!(
        freehold_orchestrator_lib::relay_member::DEFAULT_BUZZ_COMPOSE_DIR
            .starts_with(freehold_orchestrator_lib::planebase::GUEST_PATH_RELAY_DEPLOY),
...
fn backup_flag_tracks_srv_data_split() {
    use freehold_orchestrator_lib::planebase::*;
...
fn lxc_mp_args_carry_backup_flags() {
    use freehold_orchestrator_lib::planebase::MountSpec;
    let args = freehold_orchestrator_lib::drive::lxc_mp_args(&[
        MountSpec {
...
async fn storage_info_reads_live_mount_usage_and_guest_liveness() {
    ...
    let (_log, _rd, client, server) = proxmox_fixture(base.path(), &bin).await;
    let info = freehold_orchestrator_lib::drive::storage_info(
        &client,
        "proxmox-box",
        freehold_orchestrator_lib::planebase::BackendKind::Zfs,
        "rpool",
        &[
            (
                "relay".to_string(),
